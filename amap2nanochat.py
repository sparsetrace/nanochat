...
Reverse graft / continued-pretraining harness: take the AMAP-reconditioned
d32 weights from sparsetrace/d32ft and continue training them under
Karpathy's ORIGINAL nanochat attention (attn_variant="standard", FA3 path).

This is the mirror image of modal_d32ft.py:

  modal_d32ft.py      karpathy/nanochat-d32 (standard)  ->  AMAP operator
  amap2nanochat.py    sparsetrace/d32ft     (AMAP)      ->  standard operator

FIRST launch, if the output repo has no resume-capable checkpoint:
- install Karpathy's exact d32 tokenizer (the embeddings map to it)
- resolve the newest (or requested) AMAP checkpoint:
    Volume first (base_checkpoints/<seed-run-tag>/ is usually still warm from
    the d32ft run — same named Volume), else download from the seed HF repo
    (checkpoints/<seed-run-tag>/ preferred, latest/ as fallback)
- load it as WEIGHTS ONLY into a depth-32 model with attn_variant="standard"
...
WHY THE WARM START IS STRICT (unlike d32ft's allowlist): the coupled AMAP
form (witten=False) adds NO parameters — an AMAP checkpoint saved by this
fork has exactly the same state_dict structure as this fork's standard-
attention model (matrices, wte, lm_head, value_embeds, scalars/gates all
present, because the d32ft run instantiated and trained them from their
graft-neutral starting values). Any missing, unexpected, or shape-mismatched
key therefore indicates a wrong seed (vintage Karpathy file, witten=True
run, wrong tokenizer/config) and aborts. If you ever need to seed from a
witten=True run, its c_qw/c_kw keys must be dropped first — this harness
intentionally refuses to do that silently.

The checked-in scripts/base_train.py is not modified. At runtime we create an
ephemeral scripts/_amap2nanochat_base_train.py that adds one
--init-from-model argument (same mechanism as modal_d32ft.py).
...
app = modal.App("nanochat-amap2nanochat")

GPU_CONFIG = "H100"  # overridden per invocation by Function.with_options(...)

HF_REPO_DEFAULT = "sparsetrace/amap2nanochat"

# Seed: the AMAP reconditioning experiment's repo/run
SEED_REPO_DEFAULT = "sparsetrace/d32ft"
SEED_RUN_TAG_DEFAULT = "d32ft-amap"
...
image = (
    ...
    .add_local_dir(
        ...
        ignore=[
            ...
            "modal_eval.py", "amap2nanochat.py",
        ],
    )
...
def _run_streamed(cmd: str, log_path: Path | None = None):
    print(f"[a2n] running: {cmd}", flush=True)
...
def _install_warmstart_train_module() -> str:
    ...
    dst = Path(REPO_DIR) / "scripts" / "_amap2nanochat_base_train.py"
    text = src.read_text()
    ...
    print0(f"[a2n] Weights-only warm start from {args.init_from_model}")
    print0("[a2n] New optimizer + new dataloader state; local step starts at 0")
    ...
    for _k in _missing:
        print0(f"[a2n]   missing from checkpoint: {_k}")
    for _k in _unexpected:
        print0(f"[a2n]   unexpected in checkpoint: {_k}")
    _shape_bad = [
    ...
    for _k, _cs, _ms in _shape_bad:
        print0(f"[a2n]   SHAPE MISMATCH: {_k} ckpt{_cs} vs model{_ms}")
    ...
    assert not _shape_bad, \
        "[a2n] shape mismatch — wrong tokenizer or model config for this seed"
    assert not _unexpected, \
        "[a2n] seed has keys this model lacks (witten=True run?) — refusing"
    assert not _missing, \
        "[a2n] seed lacks keys this model has (vintage checkpoint?) — refusing"
    model.load_state_dict(_init_state, strict=True)
    print0("[a2n] strict load OK: AMAP weights grafted under standard attention")
    del _init_state, _own
"""
    text = text.replace(load_anchor, load_anchor + warmstart, 1)
    dst.write_text(text)
    print(f"[a2n] installed ephemeral trainer: {dst}")
    return "scripts._amap2nanochat_base_train"
...
def _ensure_source_tokenizer():
    ...
    print(f"[a2n] installed exact Karpathy d32 tokenizer -> {tok_dir}")
...
def _resolve_amap_seed(
    api, token: str, seed_repo: str, seed_run_tag: str, seed_step: int
):
    """Locate the AMAP seed checkpoint (model_*.pt + meta_*.json).

    Volume first: the shared Volume normally still holds the d32ft run's
    ...
    if local_steps and (seed_step in local_steps or seed_step < 0):
        ...
        print(f"[a2n] seed (volume, no download): {model_path}")
    ...
    if chosen_prefix is None:
        raise RuntimeError(
            f"No AMAP seed checkpoint found: neither the Volume ({seed_dir}) "
            f"nor {seed_repo} has "
            f"{'step ' + str(seed_step) if seed_step > 0 else 'any step'} "
            f"under checkpoints/{seed_run_tag}/ or latest/"
        )

    dest_dir = Path(CACHE_DIR) / "bootstrap" / "amap-seed" / f"{step:06d}"
    ...
    print(f"[a2n] seed (HF download): {origin} -> {model_path}")
    return model_path, meta_path, step, origin


def _validate_seed_meta(meta_path: Path, seed_run_tag: str):
    """The seed must be the AMAP run: depth 32, hmap alpha=0. Guards against
    grabbing a foreign checkpoint out of latest/. Metas that predate some
    fields are tolerated (None passes)."""
    ...
    if variant not in (None, "hmap"):
        raise RuntimeError(f"Seed attn_variant={variant!r} is not the AMAP run")
    alpha = uc.get("hmap_alpha")
    if alpha not in (None, 0, 0.0):
        raise RuntimeError(
            f"Seed hmap_alpha={alpha!r} != 0 — not AMAP; a nonzero Doob "
            f"sector changes what the weights were trained against"
        )
    if uc.get("witten"):
        raise RuntimeError(
            "Seed was trained with witten=True (extra c_qw/c_kw params) — "
            "this harness refuses to drop them silently"
        )
    print("[a2n] seed meta validated: depth=32, attn=hmap, alpha=0 (AMAP)")
...
def _restore_own_checkpoint(api, hf_repo: str, token: str, run_name: str):
    ...
    if best_local >= chosen_step:
        use_step = best_local
        print(
            f"[a2n] RESUME (volume, no download): step {use_step} "
        ...
        print(
            f"[a2n] RESUME (HF download): {hf_repo}/{chosen_prefix} "
    ...
    print(f"[a2n] RESUME: step {use_step} -> {ckpt_dir}")
...
def _periodic_sync(
    ...
    while not stop.wait(every_s):
        try:
            ckpt_vol.commit()
            print("[a2n] volume committed")
        except Exception as e:
            print(f"[a2n] volume commit failed (non-fatal): {e}")
        ...
        try:
            ...
            if stop.is_set():
                print("[a2n] mirror cycle skipped (training finished)")
                break
            ...
            if ops:
                ...
                print(f"[a2n] mirrored {len(ops)} file(s) -> latest/")
        except Exception as e:
            print(f"[a2n] HF mirror failed (non-fatal): {e}")
...
def upload_artifacts(
    ...
    if ops:
        api.create_commit(
            repo_id=hf_repo,
            operations=ops,
            commit_message=f"{run_tag}: standard-attn regraft through step {horizon}",
        )
        print(f"[a2n-upload] pushed {len(ops)} file(s) in one commit")
    ...
    if ckpt_dir.exists():
        ...
        for old in steps[:-keep_last_steps] if keep_last_steps > 0 else []:
            ...
            for p in list(ckpt_dir.iterdir()):
                if tok in p.name:
                    p.unlink()
            print(f"[a2n-upload] pruned superseded step {old} from Volume")
        ckpt_vol.commit()

    print("[a2n-upload] all done, returning.")
...
def train_amap2nanochat(
    hf_repo: str = HF_REPO_DEFAULT,
    run_tag: str = "amap2nanochat",
    num_gpus: int = 1,
    resume: str = "auto",
    ...
    if add_steps <= 0:
        raise ValueError("amap2nanochat requires add_steps > 0")
    ...
    print(
        f"[a2n] GPU sanity: requested={num_gpus}, "
    ...
    print("[a2n] stage: installing exact Karpathy d32 tokenizer...", flush=True)
    _ensure_source_tokenizer()
    print("[a2n] stage complete: tokenizer ready", flush=True)
    ...
    if resume != "never":
        print(
            f"[a2n] stage: checking {hf_repo} for a resume-capable checkpoint...",
            flush=True,
        )
        restored = _restore_own_checkpoint(api, hf_repo, token, run_tag)
        if restored is None:
            print("[a2n] stage complete: no own checkpoint found", flush=True)
        else:
            print("[a2n] stage complete: own checkpoint found", flush=True)
    else:
        print("[a2n] resume='never' — skipping own-checkpoint lookup", flush=True)
    ...
    if restored is not None:
        ...
        ckpt_dir.mkdir(parents=True, exist_ok=True)
        print(
            f"[a2n] stage: resolving AMAP seed "
            f"({seed_repo} / {seed_run_tag} / step {seed_step})...",
            flush=True,
        )
        seed_model, seed_meta, seed_used_step, origin = _resolve_amap_seed(
            api, token, seed_repo, seed_run_tag, seed_step
        )
        _validate_seed_meta(seed_meta, seed_run_tag)
        print("[a2n] stage complete: AMAP seed ready", flush=True)
        init_flag = f"--init-from-model={seed_model} "
        print(
            "[a2n] BOOTSTRAP: AMAP-reconditioned d32 weights -> standard "
            "attention (FA3); optimizer/dataloader start fresh at step 0"
        )
    ...
    print(
        f"[a2n] stage: ensuring {data_shards} nanochat dataset shards are available...",
        flush=True,
    )
    _run_streamed(f"{python} -m nanochat.dataset -n {data_shards}")
    print("[a2n] stage complete: dataset ready", flush=True)

    print(
        "[a2n] stage: installing ephemeral weights-only warmstart trainer...",
        flush=True,
    )
    trainer_module = _install_warmstart_train_module()
    print(f"[a2n] stage complete: trainer module = {trainer_module}", flush=True)
    ...
    # NOTE: --attn-variant / --window-pattern arrive via extra_args from the
    # workflow (VARIANT_ARGS), so the recipe lives in one place; only depth,
    # identity, cadence and horizon are pinned here. The workflow defaults to
    # --attn-variant=standard --window-pattern=L: the seed's attention
    # statistics were shaped under full-context L, and Karpathy's published
    # d32 predates the sliding-window patterns anyway.
    cmd = (
        f"{launcher} "
        f"--depth=32 "
        f"--model-tag={run_tag} "
    ...
    print(f"[a2n] origin: {origin}", flush=True)
    print(
        f"[a2n] additive budget: {add_steps} new steps "
        f"({resumed_step} -> {horizon})",
        flush=True,
    )
    print(
        "[a2n] stage: launching depth-32 standard-attention training process "
    ...
    try:
        ...
        if sync_thread.is_alive():
            print("[a2n] waiting for background mirror cycle to finish...")
        ...
        print(f"[a2n] parsed {trained_rows} step lines; {n_icl} ICL lines; "
              f"{ns} samples")
        ckpt_vol.commit()

    # Zero-step guard (resume with nothing left to do: don't re-push ~20GB).
    if trained_rows == 0 and resume_flag:
        print("[a2n] 0 training steps on a resume — skipping final upload.")
        print("[a2n] all done, returning. (Anything after this line is "
        ...
        json.dumps(
            {
                ...
                "attention": "standard (FA3)",
                "graft": "AMAP (hmap alpha=0, coupled) -> standard attention; "
    ...
    print(f"[a2n] finished in {elapsed_h:.2f} h")
    print("[a2n] all done, returning. (Anything after this line is "
          "platform teardown, not this script.)")
    return {
        "ok": True,
...
print(f"[a2n-core] source={cfg['source']} attn_variant={cfg['attn_variant']}",
      flush=True)
...
print(f"[a2n-core] CORE metric: {core['core_metric']:.4f}", flush=True)
with open(cfg["out_path"], "w") as f:
    json.dump(result, f, indent=2)
print(f"[a2n-core] wrote {cfg['out_path']}", flush=True)
'''
...
def core_eval_a2n(
    source: str = "regrafted",   # "regrafted" (standard) | "seed" (AMAP)
    hf_repo: str = HF_REPO_DEFAULT,
    run_tag: str = "amap2nanochat",
    seed_repo: str = SEED_REPO_DEFAULT,
    seed_run_tag: str = SEED_RUN_TAG_DEFAULT,
    ...
    if source == "seed":
        model_path, template_meta, use_step, _ = _resolve_amap_seed(
    ...
    if api is not None:
        ...
        api.upload_file(path_or_fileobj=str(out_path), path_in_repo=dest,
                        repo_id=hf_repo)
        print(f"[a2n-core] uploaded -> {hf_repo}/{dest}")
    print("[a2n-core] all done, returning.")
...
template = json.load(open(cfg["template_meta"]))["model_config"]
template["attn_variant"] = cfg["attn_variant"]
print(f"[a2n-sample] source={cfg['source']} attn_variant={cfg['attn_variant']}",
      flush=True)
...
with open(cfg["out_path"], "w") as f:
    ...
    f.write("\n".join(lines) + "\n")
print(f"[a2n-sample] wrote {len(lines)} samples", flush=True)
'''
...
def sample_a2n(
    ...
    with the AMAP seed available as the comparison arm. Both use the source
    ...
    if api is not None:
        ...
        api.upload_file(path_or_fileobj=str(out_path), path_in_repo=dest,
                        repo_id=hf_repo)
        print(f"[a2n-sample] uploaded -> {hf_repo}/{dest}")
    print("[a2n-sample] all done, returning.")
