...
Return-graft / continued-pretraining harness for the Transformer-hysteresis loop.
It takes the standard-attention d32 weights produced by sparsetrace/amap2nanochat
and evaluates/trains the SAME parameter vector under coupled AMAP attention
(attn_variant="hmap", hmap_alpha=0).

Path:
  Std_0  --modal_d32ft.py-->   AMAP_1
  AMAP_1 --amap2nanochat.py--> Std_1
  Std_1  --nanochat2amap.py--> AMAP_2

FIRST launch, if the output repo has no resume-capable checkpoint:
- install Karpathy's exact d32 tokenizer
- resolve the newest (or requested) Std_1 checkpoint from sparsetrace/amap2nanochat
  (shared Volume first, Hugging Face fallback)
- load it as WEIGHTS ONLY into a depth-32 coupled-AMAP model
- require strict key-for-key state_dict compatibility
- create a fresh optimizer and fresh base-data stream at local step 0

LATER launches resume this experiment's model + optimizer + dataloader state.

The coupled AMAP form adds no parameters, so the Std_1 checkpoint from this
fork must be key-for-key identical to the AMAP model. Any missing, unexpected,
or shape-mismatched key aborts.

SCIENTIFIC NOTE: measure the fixed-weight Std_1 -> AMAP_2 operator swap before
continued pretraining. The instantaneous post-graft metric is a separate
hysteresis observable from subsequent reconditioning.

IMPORTANT: the harness process uses system Python; torch work runs only under
{VENV}/bin/python subprocesses.
"""
...
app = modal.App("nanochat-nanochat2amap")

GPU_CONFIG = "H100"  # overridden per invocation by Function.with_options(...)

HF_REPO_DEFAULT = "sparsetrace/nanochat2amap"
...
# by run_tag, so this run writes to base_checkpoints/amap2nanochat/ and
# nothing collides.
...
image = (
    ...
    .add_local_dir(
        ...
        ignore=[
            ...
            "modal_eval.py", "nanochat2amap.py",
        ],
    )
...
def _run_streamed(cmd: str, log_path: Path | None = None):
    print(f"[n2a] running: {cmd}", flush=True)
...
def _install_warmstart_train_module() -> str:
    ...
    dst = Path(REPO_DIR) / "scripts" / "_nanochat2amap_base_train.py"
    text = src.read_text()
    ...
    print0(f"[n2a] Weights-only warm start from {args.init_from_model}")
    print0("[n2a] New optimizer + new dataloader state; local step starts at 0")
    ...
    for _k in _missing:
        print0(f"[n2a]   missing from checkpoint: {_k}")
    for _k in _unexpected:
        print0(f"[n2a]   unexpected in checkpoint: {_k}")
    _shape_bad = [
    ...
    for _k, _cs, _ms in _shape_bad:
        print0(f"[n2a]   SHAPE MISMATCH: {_k} ckpt{_cs} vs model{_ms}")
    ...
    assert not _shape_bad, \
        "[n2a] shape mismatch — wrong tokenizer or model config for this seed"
    assert not _unexpected, \
        "[n2a] seed has keys this model lacks (witten=True run?) — refusing"
    assert not _missing, \
        "[n2a] seed lacks keys this model has (vintage checkpoint?) — refusing"
    model.load_state_dict(_init_state, strict=True)
    print0("[n2a] strict load OK: standard-attention weights grafted under coupled AMAP")
    del _init_state, _own
"""
    text = text.replace(load_anchor, load_anchor + warmstart, 1)
    dst.write_text(text)
    print(f"[n2a] installed ephemeral trainer: {dst}")
    return "scripts._nanochat2amap_base_train"
...
def _ensure_source_tokenizer():
    ...
    print(f"[n2a] installed exact Karpathy d32 tokenizer -> {tok_dir}")
...
def _resolve_standard_seed(
    api, token: str, seed_repo: str, seed_run_tag: str, seed_step: int
):
    """Locate the standard-attention seed checkpoint (model_*.pt + meta_*.json).

    Volume first: the shared Volume may already hold the Std_1 run's
    ...
    if local_steps and (seed_step in local_steps or seed_step < 0):
        ...
        print(f"[n2a] seed (volume, no download): {model_path}")
    ...
    if chosen_prefix is None:
        raise RuntimeError(
            f"No standard-attention seed checkpoint found: neither the Volume ({seed_dir}) "
            f"nor {seed_repo} has "
            f"{'step ' + str(seed_step) if seed_step > 0 else 'any step'} "
            f"under checkpoints/{seed_run_tag}/ or latest/"
        )

    dest_dir = Path(CACHE_DIR) / "bootstrap" / "standard-seed" / f"{step:06d}"
    ...
    print(f"[n2a] seed (HF download): {origin} -> {model_path}")
    return model_path, meta_path, step, origin


def _validate_seed_meta(meta_path: Path, seed_run_tag: str):
    """Seed must be the reopened Std_1 run: depth 32, standard attention."""
    ...
    if uc.get("model_tag") not in (None, seed_run_tag):
        raise RuntimeError(
            f"Refusing seed model_tag={uc.get('model_tag')!r}; expected {seed_run_tag!r}"
        )
    variant = uc.get("attn_variant")
    if variant not in (None, "standard"):
        raise RuntimeError(f"Seed attn_variant={variant!r} is not standard attention")
    print("[n2a] seed meta validated: depth=32, attn=standard")
...
def _restore_own_checkpoint(api, hf_repo: str, token: str, run_name: str):
    ...
    if best_local >= chosen_step:
        use_step = best_local
        print(
            f"[n2a] RESUME (volume, no download): step {use_step} "
        ...
        print(
            f"[n2a] RESUME (HF download): {hf_repo}/{chosen_prefix} "
    ...
    print(f"[n2a] RESUME: step {use_step} -> {ckpt_dir}")
...
def _periodic_sync(
    ...
    while not stop.wait(every_s):
        try:
            ckpt_vol.commit()
            print("[n2a] volume committed")
        except Exception as e:
            print(f"[n2a] volume commit failed (non-fatal): {e}")
        ...
        try:
            ...
            if stop.is_set():
                print("[n2a] mirror cycle skipped (training finished)")
                break
            ...
            if ops:
                api.create_commit(
                    repo_id=hf_repo,
                    operations=ops,
                    commit_message=f"a2n mirror latest ({len(ops)} files)",
                )
                for name, mtime in names:
                    pushed_mtimes[name] = mtime
                print(f"[n2a] mirrored {len(ops)} file(s) -> latest/")
        except Exception as e:
            print(f"[n2a] HF mirror failed (non-fatal): {e}")
...
def upload_artifacts(
    ...
    if ops:
        api.create_commit(
            repo_id=hf_repo,
            operations=ops,
            commit_message=f"{run_tag}: AMAP return graft through step {horizon}",
        )
        print(f"[n2a-upload] pushed {len(ops)} file(s) in one commit")
    ...
    if ckpt_dir.exists():
        ...
        for old in steps[:-keep_last_steps] if keep_last_steps > 0 else []:
            ...
            for p in list(ckpt_dir.iterdir()):
                if tok in p.name:
                    p.unlink()
            print(f"[n2a-upload] pruned superseded step {old} from Volume")
        ckpt_vol.commit()

    print("[n2a-upload] all done, returning.")
...
def train_nanochat2amap(
    hf_repo: str = HF_REPO_DEFAULT,
    run_tag: str = "nanochat2amap",
    num_gpus: int = 1,
    resume: str = "auto",
    ...
    if add_steps <= 0:
        raise ValueError("nanochat2amap requires add_steps > 0")
    ...
    print(
        f"[n2a] GPU sanity: requested={num_gpus}, "
    ...
    print("[n2a] stage: installing exact Karpathy d32 tokenizer...", flush=True)
    _ensure_source_tokenizer()
    print("[n2a] stage complete: tokenizer ready", flush=True)
    ...
    if resume != "never":
        print(
            f"[n2a] stage: checking {hf_repo} for a resume-capable checkpoint...",
            flush=True,
        )
        restored = _restore_own_checkpoint(api, hf_repo, token, run_tag)
        if restored is None:
            print("[n2a] stage complete: no own checkpoint found", flush=True)
        else:
            print("[n2a] stage complete: own checkpoint found", flush=True)
    else:
        print("[n2a] resume='never' — skipping own-checkpoint lookup", flush=True)
    ...
    if restored is not None:
        ...
        ckpt_dir.mkdir(parents=True, exist_ok=True)
        print(
            f"[n2a] stage: resolving standard-attention seed "
            f"({seed_repo} / {seed_run_tag} / step {seed_step})...",
            flush=True,
        )
        seed_model, seed_meta, seed_used_step, origin = _resolve_standard_seed(
            api, token, seed_repo, seed_run_tag, seed_step
        )
        _validate_seed_meta(seed_meta, seed_run_tag)
        print("[n2a] stage complete: standard-attention seed ready", flush=True)
        init_flag = f"--init-from-model={seed_model} "
        print(
            "[n2a] BOOTSTRAP: reopened standard-attention d32 weights -> "
            "coupled AMAP (alpha=0); optimizer/dataloader start fresh at step 0"
        )
    ...
    print(
        f"[n2a] stage: ensuring {data_shards} nanochat dataset shards are available...",
        flush=True,
    )
    _run_streamed(f"{python} -m nanochat.dataset -n {data_shards}")
    print("[n2a] stage complete: dataset ready", flush=True)

    print(
        "[n2a] stage: installing ephemeral weights-only warmstart trainer...",
        flush=True,
    )
    trainer_module = _install_warmstart_train_module()
    print(f"[n2a] stage complete: trainer module = {trainer_module}", flush=True)
    ...
    # Pin this return leg to coupled AMAP. Optimizer/schedule overrides may
    # still be supplied through extra_args, but the operator itself may not.
    operator_args = "--attn-variant=hmap --hmap-alpha=0 --window-pattern=L "
    if "--attn-variant" in extra_args or "--hmap-alpha" in extra_args:
        raise ValueError(
            "nanochat2amap pins --attn-variant=hmap --hmap-alpha=0; "
            "do not override them through extra_args"
        )
    cmd = (
        f"{launcher} "
        f"--depth=32 "
    ...
    print(f"[n2a] origin: {origin}", flush=True)
    print(
        f"[n2a] additive budget: {add_steps} new steps "
        f"({resumed_step} -> {horizon})",
        flush=True,
    )
    print(
        "[n2a] stage: launching depth-32 AMAP training process "
    ...
    try:
        ...
        if sync_thread.is_alive():
            print("[n2a] waiting for background mirror cycle to finish...")
        ...
        print(f"[n2a] parsed {trained_rows} step lines; {n_icl} ICL lines; "
              f"{ns} samples")
        ckpt_vol.commit()

    # Zero-step guard (resume with nothing left to do: don't re-push ~20GB).
    if trained_rows == 0 and resume_flag:
        print("[n2a] 0 training steps on a resume — skipping final upload.")
        print("[n2a] all done, returning. (Anything after this line is "
        ...
        json.dumps(
            {
                ...
                "graft": "standard attention -> AMAP (hmap alpha=0, coupled); "
    ...
    print(f"[n2a] finished in {elapsed_h:.2f} h")
    print("[n2a] all done, returning. (Anything after this line is "
          "platform teardown, not this script.)")
    return {
        "ok": True,
...
# ── CORE evaluation: Std_1, fixed-weight AMAP graft, and reconditioned AMAP_2 ─
CORE_EVAL_PY = r'''
...
if cfg["attn_variant"] == "hmap":
    template["hmap_alpha"] = 0.0
    if "witten" in template:
        template["witten"] = False
print(f"[n2a-core] source={cfg['source']} attn_variant={cfg['attn_variant']}",
      flush=True)
...
print(f"[n2a-core] CORE metric: {core['core_metric']:.4f}", flush=True)
with open(cfg["out_path"], "w") as f:
    json.dump(result, f, indent=2)
print(f"[n2a-core] wrote {cfg['out_path']}", flush=True)
'''
...
def core_eval_n2a(
    source: str = "graft",       # "seed" (Std_1) | "graft" (same weights under AMAP) | "regrafted"
    hf_repo: str = HF_REPO_DEFAULT,
    run_tag: str = "nanochat2amap",
    seed_repo: str = SEED_REPO_DEFAULT,
    seed_run_tag: str = SEED_RUN_TAG_DEFAULT,
    ...
    """CORE metric on a fresh GPU.

    source="seed": evaluate Std_1 under standard attention.
    source="graft": evaluate the exact SAME Std_1 weights under AMAP (alpha=0),
                    before any optimizer update; this is the hysteresis corner.
    source="regrafted": evaluate a checkpoint produced by this AMAP_2 run.
    """
    ...
    if source in {"seed", "graft"}:
        model_path, template_meta, use_step, _ = _resolve_standard_seed(
            api, token, seed_repo, seed_run_tag, step
        )
        _validate_seed_meta(template_meta, seed_run_tag)
        if source == "seed":
            attn_variant = "standard"
            label = f"{seed_run_tag}-step{use_step}-standard"
        else:
            attn_variant = "hmap"
            label = f"{seed_run_tag}-step{use_step}-fixedweight-amap"
    elif source == "regrafted":
        ckpt_dir = Path(CACHE_DIR) / "base_checkpoints" / run_tag
        local_names = ([p.name for p in ckpt_dir.iterdir() if p.is_file()]
                       if ckpt_dir.exists() else [])
        steps = _weights_steps(local_names, "")
        assert steps, f"no AMAP_2 checkpoint on Volume under {ckpt_dir}"
        ...
        raise ValueError("source must be seed|graft|regrafted")

    workdir = Path("/tmp/n2a_core")
    ...
    if api is not None:
        ...
        api.upload_file(path_or_fileobj=str(out_path), path_in_repo=dest,
                        repo_id=hf_repo)
        print(f"[n2a-core] uploaded -> {hf_repo}/{dest}")
    print("[n2a-core] all done, returning.")
    return json.loads(out_path.read_text())


# ── Sampling: Std_1, fixed-weight AMAP graft, and reconditioned AMAP_2 ─
SAMPLE_PY = r'''
import json, sys, torch
from pathlib import Path
from nanochat.gpt import GPT, GPTConfig
...
if cfg["attn_variant"] == "hmap":
    ...
    if "witten" in template:
        template["witten"] = False
print(f"[n2a-sample] source={cfg['source']} attn_variant={cfg['attn_variant']}",
      flush=True)
...
with open(cfg["out_path"], "w") as f:
    ...
    f.write("\n".join(lines) + "\n")
print(f"[n2a-sample] wrote {len(lines)} samples", flush=True)
'''
...
def sample_n2a(
    source: str = "graft",       # "seed" | "graft" (fixed-weight AMAP) | "regrafted"
    hf_repo: str = HF_REPO_DEFAULT,
    run_tag: str = "nanochat2amap",
    seed_repo: str = SEED_REPO_DEFAULT,
    seed_run_tag: str = SEED_RUN_TAG_DEFAULT,
    ...
    with the standard-attention seed available as the comparison arm. Both use the source
    ...
    if api is not None:
        ...
        api.upload_file(path_or_fileobj=str(out_path), path_in_repo=dest,
                        repo_id=hf_repo)
        print(f"[n2a-sample] uploaded -> {hf_repo}/{dest}")
    print("[n2a-sample] all done, returning.")
